...
import { SUBSCRIPTION_STATES, Subscription, normaliseSubscriptionState } from '../../../core/models/subscription.model';
...
type SubAction =
  | { type: 'pause';   sub: Subscription }
  | { type: 'unpause'; sub: Subscription };
      ...
      mergeMap((action) => {
        if (action.type === 'subs') {
          ...
          return this.subscriptionService
            .getSubscriptions(action.handle, 20, action.token).pipe(
              catchError((err: unknown) => {
                this.store.setSubscriptionsError(extractApiError(err));
          ...
          return this.invoiceService
            .getInvoices(action.handle, 20, action.token).pipe(
              catchError((err: unknown) => {
                this.store.setInvoicesError(extractApiError(err));
